...
class WeatherFragment : BaseVmFragment<WeatherViewModel>(){
    ...
    override fun initView() {
        ...
        swipeRef.setColorSchemeColors(
            ContextCompat.getColor(requireActivity(), R.color.white),
            ContextCompat.getColor(requireActivity(), R.color.blue)
        )
        swipeRef.setOnRefreshListener { mViewModel.getRealTime(lat!!, lng!!,true) }
    }

    override fun lazyLoadData() {
        super.lazyLoadData()
        mViewModel.getRealTime(lat!!, lng!!,false)
    }
    ...
    override fun observe() {
        ...
        mViewModel.run {
            ...
            refreshStatus.observe(this@WeatherFragment, Observer {
                if (it) swipeRef.isRefreshing = false;
            })
        }
    }
